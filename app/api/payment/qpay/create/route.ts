...
export async function POST(req: NextRequest) {
    try {
        ...
        const orderUserId = order.userId || order.clientId;
        const isOwner = userId && orderUserId === userId;
        const isGuestOrder = orderUserId === "guest";

        if (!isOwner && !isGuestOrder && !order.clientId) {
        ...
        // 2. Store invoiceId in order/booking
